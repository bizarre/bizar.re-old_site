...
pub fn image(props: &Props) -> Html {
  ...
  if section.extra.contains_key("align") && section.extra.get("align").unwrap() == "left" {  
  ...
  html! {
    <>
      ...
      <div class=classes!("flex", flex_direction)>
        <div class={"flex-1"}>{ section.body.render() }</div>
        <div class={"flex-initial"}><img class={"object-contain"} src=section.extra.get("image").unwrap() /></div>
      </div>
